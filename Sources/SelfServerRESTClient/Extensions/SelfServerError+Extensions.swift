...
extension SelfServerError {
    /// A convenience initializer from a `GeneralError` response, which should have a `400`/`Bad Request` status code.
    internal init(error400 response: Components.Responses.GeneralError) {
        guard let errorCodeInt = response.headers.X_hyphen_Self_hyphen_Server_hyphen_Error_hyphen_Code,
            ...
            self.init(statusCode: .badRequest, payload: .init())
            return
        }
        
        self.init(badRequest400: errorCode, reason: try? response.body.json.reason)
    }
